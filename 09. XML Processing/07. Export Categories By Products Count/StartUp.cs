...
namespace ProductShop
{
    public class StartUp
    {
        ...
        {
            XmlRootAttribute root = new XmlRootAttribute("Categories");
            XmlSerializer serializer = new XmlSerializer(typeof(CategoriesByCountOutputDTO[]), root);

            StringBuilder builder = new StringBuilder();
            using StringWriter writer = new StringWriter(builder);

            CategoriesByCountOutputDTO[] dtos = context.Categories
                .Select(c => new CategoriesByCountOutputDTO
                {
                    Name = c.Name,
