...
namespace CarDealer
{
    public class StartUp
    {
        ...
        public static string GetTotalSalesByCustomer(CarDealerContext context)
        {
            ...
            CustomerOutputDTO[] dTOs = GetMapper()
                .Map<CustomerOutputDTO[]>(customers)
                .OrderByDescending(d => d.SpentMoney)
                .ToArray();

            XmlSerializer serializer = GetSerializer(typeof(CustomerOutputDTO[]), "customers");
        ...
        public static string GetSalesWithAppliedDiscount(CarDealerContext context)
        {
            ...
            XmlSerializer serializer = GetSerializer(typeof(SaleWithDiscountOutputDTO[]), "sales");

            SaleWithDiscountOutputDTO[] dTOs = context
                .Sales
        ...
        {
            List<CarPartOutputDTO> dTOs = context.Cars
                .OrderByDescending(c => c.TravelledDistance)
                .ThenBy(c => c.Model)
                .Take(5)
                .Select(s => new CarPartOutputDTO
                {
                    Make = s.Make,
                    Model = s.Model,
                    TravelledDistance = s.TravelledDistance,
                    Parts = s.PartCars.OrderByDescending(p => p.Part.Price).Select(p => new PartOutputDTO { Name = p.Part.Name, Price = p.Part.Price }).ToList()
                })
                .ToList();

            XmlSerializer serializer = GetSerializer(typeof(List<CarPartOutputDTO>), "cars");
            StringBuilder sb = new StringBuilder();
            using StringWriter writer = new StringWriter(sb);
